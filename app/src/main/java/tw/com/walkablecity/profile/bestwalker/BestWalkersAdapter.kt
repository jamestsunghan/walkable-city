...
class BestWalkersAdapter(val viewModel: BestWalkersViewModel) :
    ListAdapter<WalkerItem, RecyclerView.ViewHolder>(DiffCallback) {

    class ChampionViewHolder(private val binding: ItemWalkersChampionBinding) :
    ...
    class Top3ViewHolder(private val binding: ItemWalkersTop3Binding) :
        RecyclerView.ViewHolder(binding.root) {
    ...
    class WalkersViewHolder(private val binding: ItemWalkersTheRestBinding) :
        RecyclerView.ViewHolder(binding.root) {
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): RecyclerView.ViewHolder {
        return when (viewType) {
            ...
            ITEM_VIEW_TYPE_CHAMP -> ChampionViewHolder(
                ItemWalkersChampionBinding
                    .inflate(LayoutInflater.from(parent.context), parent, false)
            )
            ITEM_VIEW_TYPE_TOPS -> Top3ViewHolder(
                ItemWalkersTop3Binding
                    .inflate(LayoutInflater.from(parent.context), parent, false)
            )
            ITEM_VIEW_TYPE_WALKERS -> WalkersViewHolder(
                ItemWalkersTheRestBinding
                    .inflate(LayoutInflater.from(parent.context), parent, false)
            )
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        when (holder) {
            is ChampionViewHolder -> holder.bind(
                (getItem(position) as WalkerItem.Tops).tops[0],
                viewModel
            )
            is Top3ViewHolder -> holder.bind((getItem(position) as WalkerItem.Tops).tops, viewModel)
            is WalkersViewHolder -> {
                holder.bind(
                    (getItem(position) as WalkerItem.Walkers).walker,
                    rankingDisplay,
                    viewModel
                )
            }
